/// @script			timer_set_iterations(owner, timer_index, iterations, set_all);
/// @example		timer_set_iterations(id, 0, DIR.UP, false);
/// @description	set the number of iterations passed of a timer instance, or all timer instances
...
/// @param iterations	-- integer		-- required		-- example: 5		-- number of times the timer has completed its count cycle. Used for modulo operations
...
var _iter	= argument_count == 4 ? argument[3] : argument[2];
...
for (var i = 0; i < ds_list_size(_owner.active_timers); i++) {
	...
	if (_active_timer_index == _index) {
		_owner.active_timers[| i] = [_active_timer_time, _active_timer_dir, _active_timer_start_time, _active_timer_data, _active_timer_active, _iter];
