/// @script			timer_paused_all(owner, timer_index, list);
/// @example		timer_paused_all(obj_player, 0, my_ds_list);
...
for (var i = 0; i < ds_list_size(_owner.active_timers); i++) {
	...
	if (_timer_index == _index)
		ds_list_add(_list, !_active_timer[ACTIVE_TIMER_PROP.ACTIVE]);
}
