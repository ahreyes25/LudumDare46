/// @script			timer_get_start_time_all(owner, timer_index, list);
/// @example		timer_get_start_time_all(id, 0, _my_list);
/// @description	gets the start time of all active timer instances and puts them in a ds_list
...
for (var i = 0; i < ds_list_size(_owner.active_timers); i++) {
	...
	if (_timer_index == _index)
		ds_list_add(_list, _timer_start);
}
