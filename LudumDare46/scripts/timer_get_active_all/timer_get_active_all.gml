/// @script			timer_get_active_all(owner, timer_index, list);
/// @example		timer_get_active_all(id, 0, _my_list);
/// @description	gets the active state of all timer instances and puts them in a ds_list
...
for (var i = 0; i < ds_list_size(_owner.active_timers); i++) {
	...
	if (_timer_index == _index)
		ds_list_add(_list, _timer_active);
}
