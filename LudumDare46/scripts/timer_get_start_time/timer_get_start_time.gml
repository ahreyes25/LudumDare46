/// @script			timer_get_start_time(owner, timer_index);
/// @example		timer_get_start_time(id, 0);
/// @description	return the start time of the active timer
/// @returns		integer, the start time of the timer, or returns undefined, if the timer is not running
...
for (var i = 0; i < ds_list_size(_owner.active_timers); i++) {
	...
	var _timer_start	= _active_timer[ACTIVE_TIMER_PROP.START_TIME];
